...
<img src="https://img.shields.io/badge/Version-v8.9_Precision-715C1F?style=flat-square&logo=git&logoColor=white" alt="Version">
...
本工具專注於解決專業寫手在「撰稿、標註、交付」流程中的斷層。透過 **V8.9 獨家的行內樣式注射 (Inline Style Injection) 技術**，確保您的文章複製到 Google Docs 後，依然精準維持 **12pt 微軟正黑體** 與完美的段落間距，徹底告別格式跑版的惡夢。
...
| :--- | :--- |
| **Google Docs 字體跑版** | **[V8.9 新增]** 複製時強制注入 `font-size: 12pt` 與 `line-height: 2.0`，無論貼到哪裡，格式永遠鎖定。 |
| **關鍵字堆砌風險** | 內建 **即時密度檢測 (Density Monitor)**，以百分比視覺化關鍵字分佈，避免因過度優化遭演算法懲罰。 |
| **手動標註耗時** | 提供 **一鍵標註 (Auto Mark)** 功能，瞬間將目標關鍵字標示為顯眼的 **[茜色+粗體]**，大幅提升校對效率。 |
| **字數統計不精準** | **[V8.9 新增]** 底部狀態列支援 **互動式切換**，一鍵切換「含標點 / 不含標點」計算模式，滿足嚴格的稿件要求。 |
| **意外遺失文稿** | 內建 **防手殘機制 (Safety Lock)** 與 **自動儲存狀態燈**，防止誤觸關閉網頁導致心血白費。 |
...
* **精準字級控制**：提供 10pt-36pt 下拉選單，透過 CSS 容器偽裝技術，介面美觀且功能強大。
* **無損標註**：所有標註操作僅改變顏色與權重，**絕對不影響** 原本設定的字體大小，除非您主動調整。
...
* 選取副標題，點擊 `H2`。
* 如有需要，可使用底部狀態列切換 **「不含標點字數」** 確認篇幅。

### Step 4. 交付 (Export)
> 完成編輯後，準備交稿。
* 點擊右上方深藍色的 **[複製]** 按鈕 (會出現 Toast 提示)。
* 前往 Google Docs 按下貼上 (Ctrl+V)，您會發現字體大小完美鎖定在 **12pt**，且格式不跑版！
...
* **Version**: v8.9 (Precision Edition / 精準交付版)
